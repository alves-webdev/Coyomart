<template>
  ...
  export default {
    ...
    async fetchproducts() {
      try {
        const response = await axios.get('http://54.86.195.171:3000/api/v1/products');
        this.products = response.data;
      } catch (error) {
        console.error(error);
      }
    ...
    async deleteproduct(id) {
      try {
        await axios.delete(`http://54.86.195.171:3000/api/v1/products/${id}`);
        // Refresh the list of products after successful deletion
        this.fetchproducts();
      } catch (error) {
        console.error(error);
      }
